...
function ItemGrid:insertItem(item, xPos, yPos, isRotated)
    ...
    if stack then
        ...
        if isClient() then
            item:getModData().tetrisLockTime = getTimestampMs() + 400
        end
        self:_sendModData()
        return true
    else
...
function ItemGrid:moveStack(stack, x, y, isRotated)
    ...
    if isClient() then
        for itemID, _ in pairs(stack.itemIDs) do
            local it = self.inventory:getItemWithID(itemID)
            if it then
                it:getModData().tetrisLockTime = getTimestampMs() + 400
            end
        end
    end
...
function ItemGrid:_attemptToInsertItem_outerLoop(item, w, h, isRotated, shuffleMode)
    local itemId = item and item:getID() or 1
    local maxW = math.max(1, self.width - w + 1)
    local maxH = math.max(1, self.height - h + 1)

    local startY = shuffleMode and ((math.floor(itemId / 7)) % maxH) or (SandboxVars.InventoryTetris.EnableGravity and self.height-h or 0)
    local startX = shuffleMode and (itemId % maxW) or 0
    
    local loopForward = not shuffleMode or ((itemId % 3) == 0)
    ...
    for _,stack in ipairs(stacks) do
        for itemID, _ in pairs(stack.itemIDs) do
            ...
            if not item or not self.containerGrid:_isItemValid(item) then
                ...
                if isClient() then
                    local syncTime = TetrisClient and TetrisClient.recentlySyncedItems and TetrisClient.recentlySyncedItems[itemID]
                    if syncTime then
                        local age = getTimestampMs() - syncTime
                        if age < 2000 then -- 2 seconds grace period for network sync to catch up
                            shouldClean = false
                        else
                            -- Clean up from the recent sync list once expired
                            TetrisClient.recentlySyncedItems[itemID] = nil
                        end
                    end
                end
                ...
                if TetrisClient and TetrisClient.recentlySyncedItems then
                    TetrisClient.recentlySyncedItems[itemID] = nil
                end
            end
...
function ItemGrid:_getParentModData()
    ...
    if isoObject then
        if instanceof(isoObject, "VehiclePart") then
            isoObject = isoObject:getVehicle()
        end
        if isoObject and instanceof(isoObject, "BaseVehicle") then
...
function ItemGrid:_sendModData()
    if TetrisClient and TetrisClient.suppressSync then
        return
    end
    if isClient() then
