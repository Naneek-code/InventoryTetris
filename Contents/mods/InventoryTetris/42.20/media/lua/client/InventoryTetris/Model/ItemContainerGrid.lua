...
function ItemContainerGrid:validateCapacityRestrictions(item, container, playerObj)
    ...
    if SandboxVars.InventoryTetris.EnforceCarryWeight or isClient() then
...
function ItemContainerGrid:refresh()
    ...
    if self:hasWornItemsChanged() then
        self:refreshSecondaryGrids(true)
    end
...
function ItemContainerGrid:_updateGridPositions()
    if TetrisClient then TetrisClient.suppressSync = true end
    self.overflow = {}
    ...
    for _, itemData in ipairs(unpositionedItemData) do
        ...
        -- Try to place the item at its client-side pending placement coordinates
        local placement = TetrisClient and TetrisClient.pendingPlacements and TetrisClient.pendingPlacements[itemData.item:getID()]
        if placement and placement.container == self.inventory then
            TetrisClient.pendingPlacements[itemData.item:getID()] = nil
            local grid = self:getSpecificGrid(placement.gridIndex, placement.secondaryTarget)
            if grid then
                local oldSuppress = TetrisClient.suppressSync
                TetrisClient.suppressSync = false
                
                if grid:insertItem(itemData.item, placement.gridX, placement.gridY, placement.isRotated) then
                    placedItem = true
                    grid:_sendModData()
                end
                
                TetrisClient.suppressSync = oldSuppress
            end
        end
        ...
        gridIndex = (gridIndex % gridCount) + 1
    end

    if #remainingItemData == 0 then
        if TetrisClient then TetrisClient.suppressSync = false end
        return
    end
...
function ItemContainerGrid:refreshSecondaryGrids(forceFull)
    ...
    for target, _ in pairs(self.secondaryGrids) do
        if not items[target] then
            self:removeSecondaryGrid(target, true)
        elseif forceFull then
            self:removeSecondaryGrid(target, false)
        end
    end
...
function ItemContainerGrid:getWornItemsWithPockets()
    ...
    for i=0, wornItems:size()-1 do
        local item = (wornItems.getItemByIndex and wornItems:getItemByIndex(i))
            or (wornItems:get(i) and wornItems:get(i).getItem and wornItems:get(i):getItem() or nil)
        if item and self:_hasPockets(item) then
            items[item] = true
        end
    end
    return items
...
function ItemContainerGrid:removeSecondaryGrid(secondaryTarget, deleteModData)
    local stacks = {}
...
function ItemContainerGrid:hasWornItemsChanged()
    if not self.isPlayerInventory then
        return false
    end
    local currentItems = self:getWornItemsWithPockets()
    ...
    for target, _ in pairs(self.secondaryGrids) do
        if not currentItems[target] then
            return true
        end
    end
...
Events.OnClothingUpdated.Add(function(character)
    ...
    local grid = ItemContainerGrid._playerMainGrids[playerNum]
    if grid then
        grid:onClothingUpdated()
        local playerInv = character:getInventory()
        if playerInv and playerInv.setDrawDirty then
...
Events.OnTick.Add(function()
    for playerNum, grid in pairs(ItemContainerGrid._playerMainGrids) do
        ...
        if not player or player:isDead() or not instanceof(player, "IsoPlayer") or player:getPlayerNum() == -1 then
