...
function TetrisPocketData.getDefaultPocketDefinition(item)
    ...
    if item.getBodyLocation then
        bodySlot = item:getBodyLocation()
    end
    if (not bodySlot or bodySlot == "") and item.canBeEquipped then
        bodySlot = item:canBeEquipped()
    end

    if not bodySlot or bodySlot == "" then return nil end

    local def = TetrisPocketData.defaultPocketDefinitionsBySlot[bodySlot]
    if not def then
        local bodySlotStr = tostring(bodySlot)
        def = TetrisPocketData.defaultPocketDefinitionsBySlot[bodySlotStr]
    end
    if not def then
        local upperStr = string.upper(tostring(bodySlot))
        def = TetrisPocketData.defaultPocketDefinitionsBySlot[upperStr]
    end
    return def
end
