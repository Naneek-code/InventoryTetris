...
local function readJsonFile(fileName)
    ...
    if not reader and string.sub(fileName, -4) == ".txt" then
        local jsonName = string.sub(fileName, 1, -5) .. ".json"
        reader = getFileReader(jsonName, false)
    end
    if reader then
        local allLines = "";
...
local function writeJsonFile(fileName, json)
    ...
    if not writer then
        print("[InventoryTetris DevTool] WARNING: Could not open file '" .. tostring(fileName) .. "' for writing.")
        return
    end
...
function TetrisDevTool.copyItemData(item)
    ...
    if item:IsInventoryContainer() then
        local inv = item.getInventory and item:getInventory() or (item.getItemContainer and item:getItemContainer() or nil)
        containerData = inv and TetrisContainerData.getContainerDefinition(inv) or nil;
    end
...
function TetrisDevTool.pasteItemData(item)
    ...
    if TetrisDevTool.clipboard.itemData then
        ...
        writeJsonFile(ITEM_FILENAME..".txt", TetrisDevTool.itemEdits);
    end
    ...
    if item:IsInventoryContainer() then
        if TetrisDevTool.clipboard.containerData then
            ...
            local inv = item.getInventory and item:getInventory() or (item.getItemContainer and item:getItemContainer() or nil)
            if inv then
                local key = TetrisContainerData._getContainerKey(inv);
                ...
                if itemDef and itemDef._autoCalculated then
...
function TetrisDevTool.openEditItem(item)
    ...
    if item:IsInventoryContainer() then
        ...
        containerButton:setOnClick(function()
            local inv = item.getInventory and item:getInventory() or (item.getItemContainer and item:getItemContainer() or nil)
            if inv then
                TetrisDevTool.openContainerEdit_container(inv)
            end
        end);
...
function TetrisDevTool.openPocketEdit(item)
    ...
    if editWindow and editWindow.containerUi and editWindow.containerUi.multiGridRenderer and editWindow.containerUi.multiGridRenderer.renderers then
        local devInv = editWindow.inventory
        local renderer = editWindow.containerUi.multiGridRenderer.renderers[devInv] or editWindow.containerUi.multiGridRenderer.renderers[inventory]
        if renderer then
            renderer.previewTex = nil
            renderer.secondaryTarget = item
        end
    end
...
function TetrisDevTool.writeContainerEditsLua()
    ...
    writeText(CONTAINER_FILENAME..".txt", text);
end
