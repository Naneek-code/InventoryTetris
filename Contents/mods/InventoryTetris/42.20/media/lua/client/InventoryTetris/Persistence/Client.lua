-- Smangsty: Integrates SP hot-save and MP nested-grid persistence into Inventory Tetris core.
local OwnerResolver = require("InventoryTetris/Persistence/OwnerResolver")

local warned = {}
-- Smangsty: Prevent repeated persistence warnings from flooding the client log.
...
if TetrisClient.persistenceHooksInstalled then
    return
end
TetrisClient.persistenceHooksInstalled = true

local lastTransmittedByOwner = setmetatable({}, { __mode = "k" })
local lastTransmittedCharacterUUID = setmetatable({}, { __mode = "k" })

-- Smangsty: Sort snapshot keys deterministically for change detection.
...
-- Smangsty: Build a stable signature while excluding runtime-only grid fields.
...
-- Smangsty: Produce the comparable signature used to suppress duplicate syncs.
...
-- Smangsty: Remember the last accepted snapshot for a persistence owner.
...
-- Smangsty: Send grid data only when the persistent snapshot actually changed.
...
-- Smangsty: Include owner identity when deduplicating nested-item grid sync.
...
-- Smangsty: Cache an accepted nested-item snapshot after local or remote sync.
...
-- Smangsty: Dirty the actual SP save owner for modified inventory grid data.
...
if type(ItemGrid._sendModData) == "function" then
    ...
    -- Smangsty: Extend grid modData updates with SP hot-save dirtying.
    function ItemGrid:_sendModData(...)
        originalSendModData(self, ...)

        -- Smangsty: In SP, dirty the real save owner after grid modData changes.
...
if type(TetrisClient.transmitWorldInventoryObjectData) == "function" then
    ...
    -- Smangsty: Deduplicate world-item grid transmissions before existing MP sync.
...
if type(TetrisClient.transmitVehicleInventoryData) == "function" then
    ...
    -- Smangsty: Deduplicate vehicle grid transmissions before existing MP sync.
...
if type(TetrisClient.getInventoryContainerModData) == "function" then
    ...
    -- Smangsty: Return the real persistence owner with carried-container grid data.
...
if type(TetrisClient.transmitPlayerGridData) == "function" then
    ...
    -- Smangsty: Sync persistent character UUIDs alongside player grid data in MP.
...
if type(TetrisClient.transmitIsoObjectGridData) == "function" then
    ...
    -- Smangsty: Deduplicate furniture and IsoObject grid transmissions in MP.
...
if type(TetrisClient.transmitInventoryItemGridData) == "function" then
    ...
    -- Smangsty: Route nested-container grid sync through the authoritative owner.
...
-- Smangsty: Mark remotely applied items so normal Tetris sync does not immediately echo them.
...
-- Smangsty: Refresh an open nested grid after a remote authoritative update.
...
-- Smangsty: Safely visit cached grids when reconciling persistent remote state.
...
-- Smangsty: Record existing Inventory Tetris syncs in the persistence dedupe cache.
...
-- Smangsty: Reconcile authoritative persistence commands without swallowing core sync events.
local function onServerCommand(module, command, args)
    if module ~= OwnerResolver.MODULE then
        return
    end
    ...
    if rememberedInventoryTetrisCommands[command] then
...
-- Smangsty: Restore persisted world and vehicle grid tables received from the server.
...
-- Smangsty: Preserve or create the character UUID used by persistent search/grid ownership.
...
-- Smangsty: Ensure existing saves receive a character UUID when gameplay starts.
...
print("[InventoryTetris] Client persistence hooks installed.")
