        ...
        if item then
            ...
            if x and y then
                ...
                if not shouldCull then
                    ...
                    local lockAlpha = (item:getModData().tetrisLockTime and getTimestampMs() < item:getModData().tetrisLockTime) and 0.4 or 1
                    ...
                    local alpha = alphaMult * transferAlpha * lockAlpha
...
function ItemGridUI:renderDragItemPreview()
    ...
    -- Floor items (world items) have no source container - use a temp stack for preview
    local srcContainer = item:getContainer()
    local draggedStack
    if srcContainer then
        local otherContainerGrid = ItemContainerGrid.GetOrCreate(srcContainer, self.playerNum)
        draggedStack = otherContainerGrid:findGridStackByVanillaStack(DragAndDrop.getDraggedStack()) or ItemStack.createTempStack(item)
        -- Container hover
        ...
        draggedStack = ItemStack.createTempStack(item)
        -- Container hover - floor item, use nil as source inventory
        self:_renderPlacementPreview(hoveredStack.x, hoveredStack.y, w, h, self:getColorForStackHover(draggedStack, hoveredStack, nil, self.grid.inventory))
    end
    self:_renderControllerDrag(0.8)
end
